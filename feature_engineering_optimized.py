...
# 时序窗口参数（基于6小时间隔下采样）
TARGET_INTERVAL_HOURS = 6   # 统一采样间隔：6小时
LOOKBACK_WINDOW = 12        # 输入：过去12个时间步 = 72小时（12 × 6h）
FORECAST_HORIZON = 6        # 预测：未来6个时间步 = 36小时（6 × 6h）
...
# ==================== 数据加载 ====================
def load_raw_data():
    """加载past_2000-2022typhoons原始数据，并下采样到6小时间隔"""
    print("=" * 80)
    print("步骤1: 数据加载与6小时下采样")
    print("=" * 80)
    print(f"目标时间间隔: {TARGET_INTERVAL_HOURS} 小时\n")
    ...
    for filename in typhoon_files:
        ...
        try:
            ...
            if downsampled_length >= min_length:
                # 只保留特征列（去掉time）
                all_typhoons.append({
                    'name': filename.replace('.csv', ''),
                    'data': df_6h[BASE_FEATURES].reset_index(drop=True),
                    'original_length': original_length,
                    'downsampled_length': downsampled_length
                })
                print(f"  ✓ {filename:35s} {original_length:3d}点 → {downsampled_length:3d}点 (6h采样)")
            else:
                skipped_too_short += 1
    ...
    print(f"数据加载统计:")
    print(f"{'=' * 80}")
    print(f"✓ 成功加载: {len(all_typhoons)} 个台风")
    print(f"✗ 跳过（无time列）: {skipped_no_time} 个")
    print(f"✗ 跳过（数据太短）: {skipped_too_short} 个")
    print(f"\n下采样统计:")
    print(f"  原始总观测点: {total_original}")
    print(f"  下采样后: {total_downsampled}")
    print(f"  压缩比: {total_downsampled/total_original*100:.1f}%")
    print(f"  平均每个台风: {total_downsampled / len(all_typhoons) if all_typhoons else 0:.1f} 个观测点")
...
def save_processed_data(train_data, val_data, test_data, scalers, input_features, target_features):
    ...
    metadata = {
        ...
        'lookback_hours': LOOKBACK_WINDOW * TARGET_INTERVAL_HOURS,
        'forecast_hours': FORECAST_HORIZON * TARGET_INTERVAL_HOURS,
...
def main():
    ...
    print(f"时间间隔: {TARGET_INTERVAL_HOURS} 小时（6小时下采样）")
    print(f"时序窗口: lookback={LOOKBACK_WINDOW}步={LOOKBACK_WINDOW*TARGET_INTERVAL_HOURS}h, forecast={FORECAST_HORIZON}步={FORECAST_HORIZON*TARGET_INTERVAL_HOURS}h")
